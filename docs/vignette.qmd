...
The grid immediately narrows to three models: OLMo 7B, OLMo 2 7B, and Pythia 6.9B — all with an openness score of 5. Click OLMo 2 7B to open its profile card, which shows the foundational paper link and fetches the three most recent arXiv papers mentioning the model. If your GPU budget extends to 32 B, also remove the size cap — OLMo 2 32B and OLMo 3 32B pass every other criterion, and OLMo 3 32B also has a think variant available.
...
Six models remain: BLOOM 176B and the three Mistral models (France) and the two Falcon models (United Arab Emirates). The scatter plot makes the trade-offs immediately visible: BLOOM covers 46 languages but carries a BigScience RAIL license with restrictions on certain uses; the two Falcon models cover 4 languages each but are Apache 2.0 licensed.

> *[Screenshot: exclusion filters open with United States and China selected; scatter plot showing 6 bubbles; grid listing BLOOM 176B, Falcon 7B, Falcon 40B, Mistral 7B, Mixtral 8x7B, Mixtral 8x22B]*
...
Three models match: BLOOM 176B (46 languages, BigScience RAIL license), Falcon 7B, and Falcon 40B (4 languages each, Apache 2.0). Use the **Language (official support only)** dropdown to check whether a specific language you need is present. Selecting **Hindi** narrows the set to BLOOM 176B, Llama 3.1 8B, Gemma 3 27B, and Qwen3 8B. Selecting **Japanese** returns Gemma 3 27B, Qwen2.5 7B, and Qwen3 8B. The license column in the grid makes the trade-off immediately visible.
...
Five models appear. On the scatter plot with context window on the y-axis, three models stand out immediately — Qwen2 7B, Qwen2.5 7B, and Llama 3.1 8B all have a 131 K token context window, an order of magnitude larger than Mistral 7B and Gemma 2B. Click each model to compare licenses and training details in the profile card before deciding.

> *[Screenshot: size capped at 8 B, context window minimum at 8,192; scatter plot with Qwen2 7B, Qwen2.5 7B, and Llama 3.1 8B clearly separated from Mistral 7B and Gemma 2B]*
...
Five models score 5: OLMo 7B, OLMo 2 7B, OLMo 2 32B, OLMo 3 32B, and Pythia 6.9B. Click **OLMo 3 32B** to open its profile card — it is the only fully open model in the database that also has a chain-of-thought (think) variant available. To narrow to that model alone, also check the **Think version available** checkbox. Expand **Recent arXiv papers** to see the latest citations — useful for a quick literature check before committing to a model.

> *[Screenshot: all five openness checkboxes checked; five results in the grid; OLMo 3 32B profile card open showing Recent arXiv papers expander]*
...
#> 102 unique languages

# Filter by a single language — Japanese is supported by Gemma 3 27B, Qwen2.5 7B, and Qwen3 8B
...
Qwen2 7B, Qwen2.5 7B, and Llama 3.1 8B all offer 131 K context windows — suitable for full-document processing. Both Qwen models use Apache 2.0; Llama 3.1 carries Meta's community license, which restricts use beyond 700 M monthly users. For typical research use, any of the three works. Qwen2.5 7B is the strongest option on training scale and language coverage. If your documents are consistently under 10 K tokens, Mistral 7B is a leaner alternative.
